...
// request for leave
export const requestLeave = catchErrorAsync(async (req: CustomRequest<EmployeeDocument>, resp: Response, next: NextFunction) => {
  if(req.employee || req.admin){
  const { employeeId, from, to, gatePassTime, gatePassDate, message } = req.body;

  try {
    const employee = await EmployeeModel.findById(employeeId || req.employee?._id);
    if (!employee) {
    ...
    if (from && to) {
      leave = await LeaveModel.findOne({ employeeId: employee._id, from: fromHours, to: toHours });
      if (leave) {
        return next(new ErrorHandler("Leave already present for this date interval", 400));
      }
    }
    if (gatePassDate) {
    ...
    leave = await LeaveModel.create({ employeeId: employee._id , appliedBy:req.admin?._id || req.employee?._id, appliedDate: new Date().setHours(0,0,0,0) });
    ...
    return next(new ErrorHandler("Failed to request leave", 500));
  }}else{
...
export const getPendingLeaveRequest: RequestHandler<ParamsDictionary, any, any, Query> = async (req: CustomRequest<EmployeeDocument>, resp: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    if (req.employee || req.admin) {
      ...
      if (!isHR && !isAdmin) {
        ...
        const allPendingRequest = await LeaveModel.find({ employeeId: { $in: employeeIds }, status: 'pending', }).sort({"appliedDate":-1}).populate('employeeId').exec();
        const newRecords = []
        for(let rec of allPendingRequest){
          const docs = await EmployeeDocsModel.findOne({employeeId:(rec.employeeId as any)._id});
          if(docs){
        ...
        const allPendingRequest = await LeaveModel.find({ employeeId: { $in: employeeIds }, status: 'pending' }).sort({"appliedDate":-1})
          .populate('employeeId')
          .exec();
          const newRecords = []
          for(let rec of allPendingRequest){
...
export const getAcceptedLeaveRequest: RequestHandler<ParamsDictionary, any, any, Query> = async (req: CustomRequest<EmployeeDocument>, resp: Response, next: NextFunction): Promise<void> => {
  try {
    // Checking the jobProfile Name
    ...
    if (req.employee || req.admin) {
      ...
      if (!isHR && !isAdmin) {
        ...
        const allAcceptedRequest = await LeaveModel.find({ employeeId: { $in: employeeIds }, status: 'accepted' })
...
export const getRejectedLeaveRequest: RequestHandler<ParamsDictionary, any, any, Query> = async (req: CustomRequest<EmployeeDocument>, resp: Response, next: NextFunction): Promise<void> => {
  try {
    // Checking the jobProfile Name
    ...
    if (req.employee || req.admin) {
      ...
      if (!isHR && !isAdmin) {
        ...
        const allAcceptedRequest = await LeaveModel.find({ employeeId: { $in: employeeIds }, status: 'rejected' })
          .sort({ appliedDate: -1 })
          .skip((pge - 1) * lmt)
          .limit(lmt)
          .populate('rejectedBy')
...
// get all rejected leave Request
export const getAllLeaveAndGatePass: RequestHandler<ParamsDictionary, any, any, Query> = async (
  ...
  try {
    ...
    if (req.employee || req.admin) {
      ...
      let { name, groupName, jobProfileName, page, limit, status }: { name?: string, groupName?: string, jobProfileName?: string, page?: string, limit?: string, status?: string } = req.query;
      const pge: number = page ? parseInt(page) : 1;
      const lmt: number = limit ? parseInt(limit) : 20;
      const filter: any = {};
      const filter1: any = {};
      ...
      if (!isHR && !isAdmin) {
        ...
        const allLeaves = await LeaveModel.find({ employeeId: { $in: employeeIds } })
          .sort({ appliedDate: -1 })
          .skip((pge - 1) * lmt)
          .limit(lmt)
          .populate("acceptedBy")
...
export const getApprovedLeaveRequest : RequestHandler<ParamsDictionary, any, any, Query> = async (req: CustomRequest<EmployeeDocument>, resp: Response, next: NextFunction): Promise<void> => {
  try {
    let { name, groupName, jobProfileName, page, limit }: { name?: string, groupName?: string, jobProfileName?: string, page?: string, limit?: string } = req.query;
    const pge: number = page ? parseInt(page) : 1;
    const lmt: number = limit ? parseInt(limit) : 20;
    ...
    if (req.employee || req.admin) {
      ...
      if (!isHR && !isAdmin) {
        ...
        const ids: any = groupEmployees.map((employee) => employee._id)

        const allApprovedLeave = await LeaveModel.find({
          employeeId: { $in: ids },
          status: "approved"
        }).sort({ appliedDate: -1 })
          .skip((pge - 1) * lmt)
...
export const getLeaveRequestByEmployee = catchErrorAsync(async (req: CustomRequest<EmployeeDocument>, resp: Response, next: NextFunction) => {
  ...
  const jobProfile: JobProfileDocument | null = await JobProfileModel.findOne({ _id: req.employee.jobProfileId });
  const leaves = await LeaveModel.find({ employeeId }).populate("acceptedBy").populate("rejectedBy").sort({ appliedDate: -1 })
    .skip((pge - 1) * lmt).exec()

  const upperLevelEmployee = await EmployeeModel.findOne({ jobProfileId: jobProfile?.parentJobProfileId }).exec()
...
type JobProfileId = IJobProfile | mongoose.Schema.Types.ObjectId;

interface IEmployee extends mongoose.Document {
  // Include other properties of employee here
  jobProfileId: JobProfileId;
}

function isIJobProfile(jobProfileId: JobProfileId): jobProfileId is IJobProfile {
  return (jobProfileId as IJobProfile)?.jobRank !== undefined;
}


// update leave request status to accepted
export const updateLeaveRequestStatusAcceptedOrReject: RequestHandler<ParamsDictionary, any, any, Query> = catchErrorAsync(async (req: CustomRequest<EmployeeDocument>, resp: Response, next: NextFunction): Promise<void> => {
  ...
  try {
    ...
    const jobProfile1 = await JobProfileModel.findOne({ _id: emp?.jobProfileId, parentJobProfileId: employee?.jobProfileId });
    ...
    if (isHR || isAdmin || isSuper) {
      ...
      return next(new ErrorHandler("Not authorized to accept leave requests", 403));
    }
  } catch (error: any) {
    // Handle any potential errors and return an error response
    resp.status(500).json({
...
export const updateGatePassRequestStatusAcceptedOrReject: RequestHandler<ParamsDictionary, any, any, Query> = catchErrorAsync(async (req: CustomRequest<EmployeeDocument>, resp: Response, next: NextFunction): Promise<void> => {
...
export const approveLeaveRequestOrReject: RequestHandler<ParamsDictionary, any, any, Query> = catchErrorAsync(async (req: CustomRequest<EmployeeDocument>, resp: Response, next: NextFunction): Promise<void> => {
  const { employeeId, from, to, status, rejectedReason } = req.body;
...
// update leave Request to Approved
export const approveGatePassRequestOrReject: RequestHandler<ParamsDictionary, any, any, Query> = catchErrorAsync(async (req: CustomRequest<EmployeeDocument>, resp: Response, next: NextFunction): Promise<void> => {
  const { employeeId, gatePassDate, gatePassTime, status, rejectedReason } = req.body;
